...
public class MainActivity extends Activity implements HidService.Listener {
    ...
    @Override protected void onDestroy(){ if(discoveryRegistered){try{unregisterReceiver(discoveryReceiver);}catch(Throwable ignored){} discoveryRegistered=false;} super.onDestroy(); }

    private void ensurePermissionAndService(){
        if(Build.VERSION.SDK_INT>=31 && checkSelfPermission(Manifest.permission.BLUETOOTH_CONNECT)!=PackageManager.PERMISSION_GRANTED){
            requestPermissions(new String[]{Manifest.permission.BLUETOOTH_CONNECT, Manifest.permission.BLUETOOTH_SCAN},REQ_BT); return;
        }
        startHidService();
    }
    ...
    private void buildUi(){
        ...
        Button pair=btn("FIND COMPUTER"); body.addView(pair,new LinearLayout.LayoutParams(-1,dp(48))); LinearLayout row=new LinearLayout(this); Button refresh=btn("Refresh"); Button connect=btn("Connect"); Button disconnect=btn("Disconnect"); row.addView(refresh,weight());row.addView(connect,weight());row.addView(disconnect,weight());body.addView(row);
        ...
        pair.setOnClickListener(v->startScan()); refresh.setOnClickListener(v->refreshDevices()); connect.setOnClickListener(v->connectSelected()); disconnect.setOnClickListener(v->{HidService s=HidService.get();if(s!=null)s.disconnect();});
    ...
    private final BroadcastReceiver discoveryReceiver=new BroadcastReceiver(){
        @Override public void onReceive(Context context,Intent intent){
            if(BluetoothDevice.ACTION_FOUND.equals(intent.getAction())){
                ...
                if(d==null)return;
                if(!bonded.contains(d)){ bonded.add(d); onLog("SCAN found host="+safeName(d)); updateDeviceList(); }
            } else if(BluetoothAdapter.ACTION_DISCOVERY_FINISHED.equals(intent.getAction())) onLog("PASS SCAN discovery finished results="+bonded.size());
        }
    };
    private boolean discoveryRegistered=false;
    private void startScan(){
        HidService s=HidService.get(); if(s==null||!s.readyForPairing()){onLog("FAIL SCAN HID not registered");status.setText("Wait for HID_REGISTERED");return;}
        if(Build.VERSION.SDK_INT>=31&&checkSelfPermission(Manifest.permission.BLUETOOTH_SCAN)!=PackageManager.PERMISSION_GRANTED){onLog("FAIL SCAN permission missing");requestPermissions(new String[]{Manifest.permission.BLUETOOTH_CONNECT,Manifest.permission.BLUETOOTH_SCAN},REQ_BT);return;}
        BluetoothManager bm=getSystemService(BluetoothManager.class); BluetoothAdapter a=bm==null?null:bm.getAdapter(); if(a==null){onLog("FAIL SCAN adapter unavailable");return;}
        if(!discoveryRegistered){IntentFilter filter=new IntentFilter();filter.addAction(BluetoothDevice.ACTION_FOUND);filter.addAction(BluetoothAdapter.ACTION_DISCOVERY_FINISHED);if(Build.VERSION.SDK_INT>=33)registerReceiver(discoveryReceiver,filter,Context.RECEIVER_NOT_EXPORTED);else registerReceiver(discoveryReceiver,filter);discoveryRegistered=true;}
        bonded.clear();updateDeviceList();try{if(a.isDiscovering())a.cancelDiscovery();boolean ok=a.startDiscovery();onLog((ok?"PASS":"FAIL")+" SCAN startDiscovery accepted="+ok);status.setText(ok?"Scanning for computers…":"Scan failed");}catch(SecurityException e){onLog("FAIL SCAN SecurityException "+e.getMessage());}
    }
    private void updateDeviceList(){ArrayList<String> names=new ArrayList<>();for(BluetoothDevice d:bonded)names.add(safeName(d));devices.setAdapter(new ArrayAdapter<>(this,android.R.layout.simple_spinner_dropdown_item,names));}
    ...
    private void connectSelected(){ HidService s=HidService.get();int i=devices.getSelectedItemPosition();if(s==null||i<0||i>=bonded.size()){status.setText("Choose a computer");return;}onLog("PAIR_CONNECT user selected host="+safeName(bonded.get(i)));s.pairAndConnect(bonded.get(i)); }
